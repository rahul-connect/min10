...
?>
    ...
    <div class="container">
      ...
      <div id="heading" class="col-lg-10">
       <h1 class="text-center">PENDING APPROVAL</h1><br>
       <table id="cat_table" class="table table-responsive table-hover">
		...
		<tbody>
		<?php
		$fetch_pending = mysqli_query($con,"SELECT * FROM video WHERE status=0");
		$i = 0;
		while($row = mysqli_fetch_array($fetch_pending)){
            $id = $row['id'];
            $title = $row['title'];
             $cat_id = $row['category'];
            ...
            $i++;
			echo "
				<a href='view_pending.php?id=<?php echo $id; ?>'>
				<tr class='text-center'>
				<td>$i</td>
				<td><a href='view_pending.php?id=$id'>$title</a></td>
				<td>$category</td>
				<td>$language</td>
				<td>$username</td>
				</tr>
